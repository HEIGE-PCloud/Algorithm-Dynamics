...
Codeforces is a competitive coding platform, it is mainly used by people to hold coding competition.
...
\begin{tabulary}{\linewidth}{|L|L|}
    ...
    \hline
    The software can perform simple data analysis to the assignments data. & This provides teachers with an overview of students' performance on their assignments and allows them to help their students better.\\
    \hline
...
The AssignmentPage is where the user interacts with the assignments. For students, they can work on their assignments; while teachers can create and mark assignments.

\subsubsection{Usability feature}

The user can switch between different tabs by clicking the navigation bar at the top. They can enter the detailed view of an assignment by clicking one assignment. A dropdown button is placed on the top right for the user to create or import a new assignment.
...
If the user has login to his Microsoft account, the avatar, username, email and role will be managed by his Microsoft Account, the edito button will navigate to Microsoft account website, and the login button will be replaced to a logout button. If the user has login locally, he can edit his information by clicking the edit button and login to Microsoft account by clicking the login button.

Some interesting statistics will be displayed in the button.

\subsubsection{Validation}

When the user edit his local identity, the user name will not be allowed to exceed 32 characters, and a format check will be applied to the email address.
...
The CreateNewProblemPage is used to create new problem. It contains multiple textboxes for the user to input problem data.

This page will be reused for editing existing problems. The title will be altered correspondingly.

\subsubsection{Usability feature}

The Tags field will be a tokenizing text box with auto suggestion to make input faster. The Difficulty field will be a drop down list for easy selection. The Time Limit and Memory Limit field will be a number box. The user can add new test case by clicking the add button and remove existing ones by clicking the delete button. If the user click the cancel button, a flyout will be display to confirm cancelation.

\subsubsection{Validation}

There are many input boxes in this page, they will all be validated. For the Problem Name field, the user can enter 32 characters at maximum. In the Tags field, the user can enter 3 tags at maximum. In the Time Limit and Memory Limit field, the user can only enter numbers.
...
The CreateNewProblemListPage is used to create new problem list. It contains multiple textboxes for the user to input problem list data.

The page will be reused for editing existing problem lists. The title will be altered correspondingly.

\subsubsection{Usability feature}

There will be an auto suggest box for the user to search for problem and add them to the list. There is a export and a save button at the button for the user to export or save the problem list. If the user click the cancel button, a flyout will be display to confirm cancelation.

\subsubsection{Validation}

For the List Name field, the user can enter 32 characters at maximum.
...
The CreateNewAssignmentPage is used to create new Assignment. It contains multiple textboxes for the user to input assignment data.

The page will be reused for editing existing assignment as well. The title will be altered correspondingly.

\subsubsection{Usability feature}

The due date will be a datetime picker, a visual calendar will be displayed for the user to pick a due date. Same auto suggest box stuff.

\subsubsection{Validation}

The name will be validate, the due date will be validated by the picker.
...
The Judger will take a few seconds to judge each \mintinline{text}{Submission}, while it is juding, the other parts of the software need to keep working on their tasks. So I use concurrent processing to let the Judger run parallel with the main program.

When judging a single \mintinline{text}{TestCase}, the Judger will first start the compiler in a new process to compile the user's code. The Judger redirects the output of the compiler to an internal string variable to store all outputs. When the compiler finishes compiling, an interrupt is triggered and the Judger will be notified that the compilation has been done. It will check the compiler's exit code and its output, if there is any compilation errors, they will be reported back. Otherwise, the Judger will run the compiled executable, and feed the input to its \mintinline{text}{stdin} and redirects all \mintinline{text}{stdout} and \mintinline{text}{stderr} to internal string variables. On top of that, the Judger will start a countdown timer in a different process to track the time. When the countdown reaches zero, an interrupt will be sent to the Judger and it will terminate user's code and report a time limit exceed error, or if the user's code finishes first, then its output will be compared with the expected output, the result and all statistics will then be reported.
