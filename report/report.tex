...
I choose to use SQLite to power this software. It is a small, self-contained, high-reliability, full-featured SQL Database engine, which will be enough to handle all the data storing and querying requests. I choose to use Microsoft.Data.SQLite for the database interface, which allows me to send SQL requests to the database. Details about the design of the database and queries will be described in the \hyperref[subsec:database design]{Database design section}.
...
This algorithm is designed for the \mintinline{text}{Judger} to run a piece of code, pass input to the code and receive all the output and error. This is the basic function of the \mintinline{text}{Judger} and further judging will all base on this algorithm. An async function will be used so it will not block the main UI thread. It takes three input, \mintinline{text}{UserCode} for the code to be executed, \mintinline{text}{TestCase} for the input data, and language for the programming language configuration. There are two types of programming languages, interpreted programming languages and complied programming languages, the procedure to run them is quite different, so I need to handle them separately.
...
Before the Judger can run the code, it needs to first save the source code into a local temperate file for the compiler or the interpreter to read it. So I design a \mintinline{text}{SetSourceCodeFilePath} procedure to allow the folder to be set.

\begin{minted}[linenos,tabsize=4,breaklines]{text}
public static partial class Judger
{
    ...
    {
        this._SourceCodeFolderPath = FolderPath
        this._SourceCodeFilePath = FolderPath + FileName + ".txt"
        this._ExecutableFilePath = FolderPath + FilePath + ".exe"
    }
}
\end{minted}
...
Next, I need to implement the \mintinline{text}{Compile} function, which runs the compiler in another process concurrently and gets all outputs. This function will be a private method to prevent it from being called accidentally. Because it needs to run concurrently, I use an async method and connect two helper procedures to handle the \mintinline{text}{OutputDataReceived} and \mintinline{text}{ErrorDataReceived} events. All outputs received will be stored in private strings \mintinline{text}{_CompilationOutput} and \mintinline{text}{_CompilationError}. An exit code will be returned by the \mintinline{text}{Compile} function. If the exit code is 0, it means the compilation is executed successfully, if it is some other number, it means something wrong has happened. The \mintinline{text}{RunCode} function will rely on this return value to determine what to do next.
...
public static partial class Judger
{
    ...
    {
        // Validate the data before recording
    ...
    private async static Task<int> Compile()
    {
        ...
        // Bind the events to helper procedures
...
After compilation has been completed, I need to run the executable and pass the input to it. So I design another procedure \mintinline{text}{Execute} to run the code. Similarly, there are two variables \mintinline{text}{_StandardOutput} and \mintinline{text}{_StandardInput} used to store the outputs from the user's code. On top of this, a timer needs to be started when the user's code starts to run. The timer will kill the user's code if it does not end in a certain period and report a time-limit-exceed error to the judger. A status code is designed to track the execution status of user's code.
...
public static partial class Judger
{
    private StatusCode _StatucCode
    ...
    {
        // Only successfully finished when the status code is not TIME_LIMIT_EXCEEDED
    ...
    private async static Task<int> Execute(string Input, Language language, int TimeLimit)
    {
        ...
        // Bind all events to helper procedures
        ...
        // Create the timer for timing
        Timer timer = new Timer(delegate 
        {
            // If user's code is still running when the timer has finished
...
Finally, I will create an exposed \mintinline{text}{RunCode}.
...
public static partial class Judger
{
    public async static Task<TestCaseResult> RunCode(string UserCode, TestCase testCase, Language language, int TimeLimit)
    {
        ...
        {
            // If compile error, return the error
        ...
        if (exitCode == 0 && _StatusCode == StatusCode.FINISHED)
        {
...
public static partial class Judger
{
    public async static Task<SubmissionResult> JudgeProblem(Problem problem, Submission submission, Language language)
    {
        ...
        // Create JudgeQueue
        Queue<TestCase> JudgeQueue = new Queue<TestCase>();
        // Push All TestCases to the queue
        for i=0 to len(problem.TestCases)
            JudgeQueue.Enqueue(TestCase)
        next i

        // Go through all TestCases in the queue
...
public static partial class Judger
{
    public async static Task<SubmissionResult> JudgeAssignment(Assignment assignment, AssignmentSubmission assignmentSubmission, Language language)
    {
        ...
        for i=0 to len(assignment)
            ProblemQueue.Enqueue(assignment[i])
            SubmissionQueue.Enqueue(assignmentSubmission.Submissions[i])
        next i
        while len(ProblemQueue) > 0 and len(SubmissionQueue) > 0
...
The \mintinline{text}{RunCodeTimeLimit} field stores the max time allowed for the run code function. The data will be validated before applying, if it is not a positive integer, then a default 1000 will be applyed and write back to this field. 
...
The \mintinline{text}{LanguageConfiguration} is a list of dictionary allowing the user to configure custom programming languages. The data will be validated before applying, if a configuration is invalid, it will be ignored.
...
The Submission table will have an auto-increment integer \mintinline{text}{Id} as its primary key. Since SQLite does not support the \mintinline{text}{DateTime} data type, the time will be converted to a string before storing. Similarly, conversion will be done for Language and SubmitterId.
...
\subsection{Milestone 5: Create database}
