...
Timofei thinks putting the links on the same page makes it messy. He thinks those websites and resources can be easily find on the Internet, so there is no need to put extra links on the HomePage.
...
Timofei thinks there can be more interesting statistics, such as favourite variable names, and the total time spend in the software.
...
Timofei likes the idea of being able to switch between a light theme and a dark theme. He thinks it will be better if users are allowed to create their custom theme. He also suggests that allowing the user to change the font size manually may help a lot.
...
The Problem table will have \mintinline{text}{Uid} as its primary key. Both \mintinline{text}{Status} and \mintinline{text}{Difficulty} will be stored in integer for the same reason before.
...
\begin{tabulary}{\textwidth}{|L|L|L|}
    ...
    Id & ProblemUid & TagId \\
    \hline
    1 & \mintinline{text}{<GUID>} & 1 \\
    \hline
    2 & \mintinline{text}{<GUID>} & 2 \\
    \hline
\end{tabulary}
...
(
    ...
    FOREIGN KEY (ProblemUid) REFERENCES Problem(Id),
...
(
    ...
    FOREIGN KEY (ProblemUid) REFERENCES Problem(Uid)
)
\end{minted}
...
\begin{tabulary}{\textwidth}{|L|L|L|L|}
    \hline
    Id & ProblemUid & Code & Time \\
    \hline
    1 & \mintinline{text}{<GUID>} & ``source code'' & ``11/11/2021 14:20:04`` \\
    \hline
\end{tabulary}
...
The ProblemList table will have an auto-increment integer \mintinline{text}{Id} as its primary key. The description is allowed to be empty for easy sharing.
...
\begin{minted}[linenos,tabsize=4,breaklines]{json}
{
    ...
    "DataType": "Problem",
    "Data": {
        "Uid": "2b2bc7d4-603f-4af0-b77f-d6395d9997cd",
        "Name": "Problem 1",
        "Difficulty": 2,
...
When the problem is imported, if the problem already exists, its data is updated, otherwise, a new record is created.

The problem and test cases are imported first.

\begin{minted}[linenos,tabsize=4,breaklines]{sql}
INSERT OR REPLACE INTO Problem VALUES (Uid, Name, Difficulty, Description, TimeLimit, MemoryLimit)
INSERT OR REPLACE INTO TestCase VALUES (Input, Output, IsExample, Uid)
\end{minted}
...
INSERT INTO TagRecord VALUES (Uid, last_insert_rowid)
\end{minted}
...
\begin{minted}[linenos,tabsize=4,breaklines]{json}
{
    ...
    "Data": {
        ...
        "Problems": [
            {
                "Uid": "da58f27b-e1f6-4d12-8137-6054677c91cb"
                "Name": "Problem 1",
                "Difficulty": 2,
                "Description": "Problem description 1",
                "TimeLimit": 1000,
            ...
            {
                "Uid": "76ef1041-9a2a-4672-af34-c3f76b0fdeac",
                "Name": "Problem 2",
                "Difficulty": 0,
...
When a problem list is imported, all problems are imported in the same way. Then the information about the problem list itself is imported and the ProblemListRecord table is updated.

\begin{minted}[linenos,tabsize=4,breaklines]{sql}
INSERT OR REPLACE INTO ProblemList VALUES (Name, Description)
INSERT OR REPLACE INTO ProblemListRecord VALUES (Uid, last_insert_rowid)
\end{minted}
...
\begin{tabulary}{\textwidth}{|L|L|L|}
    ...
    \hline
    Click different buttons on the NavigationView. & The main frame switches to the correct page. & A normal test to make sure the NavigationView is working correctly. \\
    \hline
    Click different buttons on the MainPage. & The main frame switches to different functions correctly. & A normal test to make sure the HomePage is working correctly. \\
    \hline
...
\begin{tabulary}{\textwidth}{|L|L|L|}
    ...
    \hline
    No settings file. & A default settings file is generated. & An extreme test simluates the suitation when the software is first installed and there is no existing settings file. A correct default settings file must be created correctly. \\
    \hline
    A settings file with incorrect JSON format. & A default settings file overrides the incorrect file. & An erroneous test to make sure the settings module can process files with incorrect format. \\
    \hline
...
\begin{tabulary}{\textwidth}{|L|L|L|}
    ...
    \hline
    Login with a teacher account, navigate to the assignment page. & All local assignments and remote assignments create by the teacher show up correctly. & A normal test to make sure the assignments can be fetched correctly. \\
    \hline
    ...
    \hline
    Click the logout button on the Account page. & The user gets logout correctly. & A normal test to make sure the user can logout correctly. \\
    \hline
\end{tabulary}
...
Any failed test cases will be investigated and at the end of the alpha testing, all unit tests must be passed with a code coverage higher than 90\%.
