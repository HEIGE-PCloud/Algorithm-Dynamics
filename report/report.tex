...
Now, it is ready to run the compiler. I first clear the old outputs, then set up the arguments for the compiler, next bind the two event handlers to the process and finally start the compile process.
...
\mintinline{text}{StatusCode} has four possible values, \mintinline{text}{PENDING} is set when the code is waiting to be run. When it starts executing, the status code will be set to \mintinline{text}{RUNNING}. If it finishes in time, its status will be set to \mintinline{text}{FINISHED}. If it needs to be killed by the Judger, a \mintinline{text}{TIME_LIMITED_EXCEEDED} status will be recorded.

Similarly, I will use event handlers to record all the output from the user's code. I add a third handler to process the exit event, which will set the \mintinline{text}{_StatusCode} mentioned above.
...
The \mintinline{text}{Execute} function has a similar design with the \mintinline{text}{Compile} function, but it starts a timer concurrently to count the time and kill the process if needed.
...
Now I have everything I need to build the \mintinline{text}{RunCode} function. It takes in the user's code, input data, language configuration and time limit, outputs a test case result containing all the running information.
...
partial class Judger
    public function JudgeTestCase(string UserCode, TestCase TestCase, Language Language, int TimeLimit)
        ...
        TestCaseResult result = RunCode(UserCode, TestCase.Input, Language, TimeLimit)
...
This algorithm is designed to judge a submission. I set up a judging queue and push all test cases in to the queue. Then I take them out and judge them one by one. A queue is suitable for this usecase because I want the first test case entered to be judged first. A submission result will be returned at the end.
...
partial class Judger
    public function JudgeProblem(Submission Submission, Language Language)
        ...
        // Create the judge queue
        Queue JudgeQueue = new Queue(Submission.Problem.TestCases)

        // Keep judging until the queue is empty
        while len(JudgeQueue) > 0
            result.TestCaseResults.Append(JudgeTestCase(Submission.Code, JudgeQueue.Dequeue(), Language, Submission.Problem.TimeLimit))
        endwhile
        return result
    endfunction
endclass
...
This algorithm is designed to judge an assignment submission. I set up a judging queue and push all problems in to the queue. Then I take them out and judge them one by one. A queue is suitable for this usecase because I want the first test case entered to be judged first. An assignment submission result will be returned at the end.
...
partial class Judger
    public function JudgeAssignment(Assignment Assignment, AssignmentSubmission AssignmentSubmission, Language Language)
        ...
        // Keep judging until the queue is empty
        while len(JudgeQueue) > 0
            result.SubmissionResults.Append(JudgeProblem(SubmissionQueue.Dequeue(), Language))
        endwhile
        return result
    endfunction
endclass
