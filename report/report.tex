...
The AssignmentSubmission table will have \mintinline{text}{Uid} as its primary key. It has a one-to-one relationship with a AssignmentSubmission, so a \mintinline{text}{Id} field is used to record this relationship.
...
I create all pages under the \mintinline{text}{Algorithm_Dynamics.Pages} namespace. For now, all of them will only contain a text block showing their name. Here is an example of the HomePage.
...
I need to add the items to the collection when the HomePage is loaded. I create a procedure \mintinline{text}{InitializeQAItems}. Because I have passed the MainWindow object to the function, the MainNavView can be called inside. Because the CodingPage and the import/export logic is not completed yet, the Random Problem and Import function will be implemented later.
...
As I expected, the WebView2 process is created not destroyed when I navigate away from the PlaygroundPage. And each time a new instance is create, which is causing the memory leak. After some researching on the lifecycle of the WebView2 component \cite{github:microsoft-ui-xaml:4752}, I found that I can call \mintinline{text}{WebView2.Close()} to manually dispose it. So I add a new procedure to handle the control unload event.
...
Since the CodeEditor is implemented as a user control, I can reuse it easily in the CodignPage.

TODO: write the report for the CodingPage
